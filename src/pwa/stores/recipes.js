...
export const useRecipesStore = defineStore('recipes', () => {
  ...
  // Nouvelle méthode pour forcer le rafraîchissement - cache désactivé, même comportement que fetchRecipes
  async function fetchRecipesFresh(params = {}) {
    loading.value = true
    error.value = null

    try {
      console.log('🔄 [RecipesStore] Rafraîchissement forcé des recettes (cache désactivé)')
      const response = await apiService.getRecipes(params) // Cache désactivé, même résultat
  ...
  // Nouvelle méthode pour forcer le rafraîchissement d'une recette - cache désactivé, même comportement que fetchRecipe
  async function fetchRecipeFresh(id) {
    loading.value = true
    error.value = null

    try {
      console.log('🔄 [RecipesStore] Rafraîchissement forcé de la recette:', id, '(cache désactivé)')
      const response = await apiService.getRecipe(id) // Cache désactivé, même résultat
